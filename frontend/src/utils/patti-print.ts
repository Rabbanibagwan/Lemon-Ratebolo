...
export function renderPattiHtml(p: Patti, profile: ShopProfile, qrUri: string, userName: string, detailed: boolean = false): string {
  ...
  <div class="card">
    ...
    <div class="qr">
      <img src="${qrUri}"/>
      <div><div class="qrLbl">SCAN AT COUNTER</div><div class="qrHint">Scan to open this Patti and enter/update the receiver name.</div></div>
    </div>
  </div>
...
/** 58 / 80 / 100 mm thermal receipt — mirrors on-screen Patti preview structure. */
export function renderThermalPattiHtml(
  ...
  // Same long date as App Preview (patti/[id].tsx).
  const date = new Date(p.created_at).toLocaleDateString("en-IN", { day: "2-digit", month: "long", year: "numeric" });
  ...
  // Match App Preview: shop name shown UPPERCASE.
  ...
  <div id="slip" class="patti">
    ...
    ${qrUri
      ? `<div class="qrbox"><img class="qr" src="${qrUri}" alt="QR"/><div class="qrtext"><div class="qrLbl">SCAN AT COUNTER</div><div class="qrHint">Scan to open this Patti and enter/update the receiver name.</div></div></div>`
      : ""}
  </div>
  </body></html>`;
}
