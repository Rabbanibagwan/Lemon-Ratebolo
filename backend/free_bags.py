...
AVAILABLE allocations never affect usable wallet balance.
...
ALLOC_STATUSES = ("AVAILABLE", "CLAIMED", "EXPIRED", "CANCELLED")
...
async def free_summary_for_shop(db, shop_id: str, *, free_used: int = 0) -> dict:
    ...
    async for row in cur:
        ...
        st = row.get("status")
        if st in ("AVAILABLE", "CLAIMED", "EXPIRED", "CANCELLED"):
            allocated += bags
        if st == "CLAIMED":
            claimed += bags
        if st == "AVAILABLE":
...
def register_free_bag_routes(
    ...
    *,
    ...
    async def admin_create_free_allocation(
        ...
        await ensure_wallet(payload.shop_id)
        now = _utc_now()
        label = period_label(payload.year, payload.month)
        alloc_id = _uid()
        doc = {
            "id": alloc_id,
        ...
        notif = {
            ...
            "shop_id": payload.shop_id,
            "kind": "FREE_BAGS",
            "title": f"You have received {int(payload.bags)} free bags",
            "body": (
                f"🎁 You have received {int(payload.bags)} free bags ({label}). "
                "Claim them now to add them to your Bag Balance."
            ),
    ...
    async def admin_list_free_allocations(
        ...
        if status:
            ...
            if st not in ALLOC_STATUSES:
                raise HTTPException(400, f"Invalid status. Use one of {', '.join(ALLOC_STATUSES)}")
            filt["status"] = st
        if year is not None:
            filt["year"] = int(year)
    ...
    async def admin_search_merchants_for_free_bags(
        ...
        """Lightweight merchant picker for free-bag allocation (no secrets)."""
        term = (q or "").strip()
        filt: Dict[str, Any] = {}
        if term:
            filt["$or"] = [
        ...
        cur = (
            db.shops.find(filt, {"_id": 0, "password_hash": 0, "id": 1, "shop_name": 1, "username": 1, "owner_name": 1, "mobile": 1})
            .sort("shop_name", 1)
            .limit(limit)
        )
        out = []
    ...
    async def merchant_claim_free_allocation(allocation_id: str, user=Depends(owner_only)):
        ...
        # Atomic claim — only one concurrent request can transition AVAILABLE → CLAIMED.
        now = _utc_now()
        claim_ref = f"FREE-{allocation_id.replace('-', '')[:10].upper()}"
        claim_txn = _uid()
        claimed = await db.bag_free_allocations.find_one_and_update(
            {"id": allocation_id, "shop_id": shop_id, "status": "AVAILABLE"},
            {
                "$set": {
                    "status": "CLAIMED",
                    "claimed_at": now,
